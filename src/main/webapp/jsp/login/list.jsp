...
<button style="width: 120px" type="button" class="layui-btn lay-submit" id="getCheckedValue">获取多选框的值</button>
</body>
    ...
    $(document).ready(function () {
        layui.use(['layer', 'form', 'table', 'laydate'], function () {
            ...
            $("#getCheckedValue").click(function () {
                ...
                console.info(table)
                var datas = table.cache["jobList"];
                for(var i = 0;i<datas.length;i++){
                    var index = datas[i]["LAY_TABLE_INDEX"];
                    var check_box=$(".layui-table-box tr[data-index="+index+"] input[type='checkbox']");
                    if(datas[i]["username"]==123){
                        check_box.prop("checked",true);
                        datas[i].LAY_CHECKED=true
                    }
                   form.render()
                }
                var aa =  table.checkStatus("jobList").data
                console.info("----aa----",aa)
            })
